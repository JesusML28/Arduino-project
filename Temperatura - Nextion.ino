...
uint8_t status = 0;
//Definicion de PID
udouble8_t Setpoint = 35;         // Valor deseado del proceso
...
void loop() {
  ...
  while (agua() == 1) {                                     // Se revisa entre cada ejecución del programa Sí el sistema cuenta con agua, mientras exista la suficiente agua el sistema continua su ejecución.
    if (digitalRead(switch_mode) == HIGH && status == 0) {  // Entramos al modo de ejecución donde se ejecuta toda la operacion del PID.
...
void new_setpoint(udouble8_t8_t* sp) {
  uint8_t setpot = map(analogRead(pot), 0, 1023, 30, 40);  // Rango de Setpoint 30 y 40 °C totalmente ajustable donde 30°C es la temperatura ambiente del agua en Barranquilla
...
void mostrar_nextion(uint8_t sensor) {
  //Sensor
  Serial.print("sv_t.val=");
  Serial.print(sensor);
  Serial.write(0xff);
  Serial.write(0xff);
  Serial.write(0xff));
  //Setpoint
  Serial.print("setpoint_t.val=");
...
void lcd_display(String md, String st) {
  ...
  uint8_t t = sensorDS18B20.getTempCByIndex(0);
  lcd.print(t);
  lcd.print(" ");
  lcd.setCursor(6, 1);
  lcd.print(st);
